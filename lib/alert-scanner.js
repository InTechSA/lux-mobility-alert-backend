...
class AlertScanner {
    ...
    Mongo.database.collection(collection, (err, collection) => {
      ...
      collection
        ...
        .each((err, doc) => {
          ...
          this._send(doc, (err) => {
            if(err) console.log(`< AlertScanner.scan > An error occurred (${err}) during alert (${JSON.stringify(doc)}) sending !`);
  ...
  _send(alert, next){
    if(!alert) return next('Alert is not an object');
    ...
    switch(alert.type) {
      case 'PUSH':
        ...
        Notifier.sendPersonalAlert(alert, function(err) {
